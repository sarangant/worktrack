  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted">Opdater dine oplysninger.</p>
      </div>

      <Card title="Profiloplysninger">
        <div className="space-y-4">
          <div>
            <label className="text-sm text-muted">Navn</label>
            {editing ? (
          ...
          <div>
            ...
            <p className="text-white">{auth.user?.role === 'admin' ? 'Administrator' : 'Medarbejder'}</p>
          </div>
          <div>
            <label className="text-sm text-muted">Bruger-ID</label>
            <p className="text-white">{auth.user?.id}</p>
          </div>
